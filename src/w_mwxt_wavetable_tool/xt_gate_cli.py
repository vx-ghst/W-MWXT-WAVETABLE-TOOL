...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME,
        description=(
            "Build and analyze the CODE V7-A Microwave XT 64/128-point reconstruction gate."
        ),
    )
    ...
    build_parser = sub.add_parser(
        "build",
        help="Build three discriminating WAVD probes and an exact restore bundle",
    )
    ...
    build_parser.add_argument(
        "--stem", default="CODE_V7_A_XT_RECONSTRUCTION_GATE"
    )

    analyze_parser = sub.add_parser(
        "analyze",
        help=(
            "Verify the 64 stored samples and optionally score independent 128-point observations"
        ),
    )
    ...
    restore_parser.add_argument(
        "--stem", default="CODE_V7_A_XT_RECONSTRUCTION_GATE_RESTORE"
    )
    return parser
                ...
                json.dumps(
                    {
                        ...
                        "target_wave_end": (
                            build.plan.target_wave_start + len(build.plan.probes) - 1
                        ),
                        ...
                        "critical_boundary": (
                            "Exact WAVD read-back proves 64 stored samples only; an independent "
                            "128-sample observation is required for the reconstruction verdict."
                        ),
                    },
                    indent=2,
                    ensure_ascii=False,
                ...
                json.dumps(
                    {
                        ...
                        "observation_complete": result.analysis.observation_complete,
